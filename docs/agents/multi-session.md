...
One GitHub issue of `emm3000/justchill-ios` labelled `ready-for-agent`. The label vocabulary is `docs/agents/triage-labels.md`, the `gh` operations are `docs/agents/issue-tracker.md`, the open board is `gh issue list --label ready-for-agent`, never a list in a doc. The issue IS the work: a `Done when` list of falsifiable conditions plus at most 3 lines of context. Constraints that outlive an issue live in `.claude/rules/` and the package `CLAUDE.md` files. Git plus engram are the chronicle; a fact goes in exactly one place. New issues come from the `ticket-writer` agent, from the PRD and the ADRs; `mattpocock-skills:grill-with-docs` stress-tests a plan before it is ticketed, and `mattpocock-skills:to-tickets` splits an approved plan. Read an issue with `gh issue view <n> --comments`, never the bare form: it prints the body alone, and the corrections to a stale body live in the comments. Proving something is absent takes every source, not the first one. *Why: on 2026-09-18 a peer reported a doc reference missing from #90 after reading only the body, where a comment cited it.*

## The loop

```
1. Orchestrator SCOPES the issue (inline, cheap) and picks the dispatch-log row
2. /wave boots one peer per ticket → WRITER   ── own worktree, own simulator, branch, commits, PR that closes the issue
3. CI runs scripts/gate on the PR
...
- Skill routing: an unknown-cause bug gets `mattpocock-skills:diagnosing-bugs` first; reading legwork goes to `mattpocock-skills:research`; a diff that is not a PR gets `mattpocock-skills:code-review`; an architecture or domain decision loads `mattpocock-skills:domain-modeling`; incoming issues go through `mattpocock-skills:triage`; behavior work loads `mattpocock-skills:tdd`.
...
- Waves are ordered by dependency; parallelism is safe only within a wave (shared `CoreDomain` or atom changes first, then independent slices). No wave carries two tickets touching the same package; a schema change (a new migration) is always a wave of one. Two tickets that both edit `project.yml` or `App/` conflict even across packages, so they also go in separate waves. A wave starts only after the previous one is merged.
...
- The file list (`fd -e swift`) and their tests, plus the pre-scoped blockers. For a move/refactor, a dependency-closure directive: transitive imports mapped from the consuming package, non-platform dependencies co-moved, platform symbols hoisted to closures.
- Gates: `JUSTCHILL_SIMULATOR=<udid> scripts/gate` green before every commit (the gate's definition: `.claude/rules/github-workflows.md`); conventional commits; no `Co-Authored-By` (a PreToolUse hook in `.claude/settings.json` blocks it); English code and docs, Spanish only for UI strings; `rg` / `fd` / `bat` / `sd`; gotchas saved to engram.
- For any ticket that adds or changes behavior (a use case, a model rule, a derived state property), an instruction to load `mattpocock-skills:tdd` and work red, green, refactor, writing the failing behavior test before the code. Pure layout or restyle tickets skip it.
- For any screen-touching ticket, a visual check on the peer's own simulator: build for it with `JUSTCHILL_SIMULATOR=<udid> scripts/build build -derivedDataPath build/DerivedData`, install with `xcrun simctl install <udid> build/DerivedData/Build/Products/Debug-iphonesimulator/justchill.app`, launch with `xcrun simctl launch <udid> com.emm.justchill`, screenshot every changed screen and state with `xcrun simctl io <udid> screenshot <file>`, and compare against `.claude/rules/ui-components.md`. Never `booted` as the device argument: more than one simulator is booted at a time. The dispatch overrides a ticket body that still names the owner's simulator. Publish the screenshots on a branch named `assets/<N>-visual-check`, with the PR head short SHA in every file name (`home-<sha>.png`), and link them in the PR body with `raw.githubusercontent.com` URLs; the `gh` CLI cannot attach images to a PR. Add the image files explicitly, never `git add -A`: `git checkout --orphan` leaves the previous branch's working files on disk, so a sweep drags the whole repo onto the assets branch. The assets branch is deleted when the cycle closes. Evidence a criterion turns on goes in the PR body in full, not summarised there and pasted to the orchestrator: the reviewer reads the PR. When a criterion is a measurement, such as a frame from the view hierarchy, quote the lines rather than the conclusion drawn from them.
...
- Each worktree generates its own `justchill.xcodeproj` and gets its own DerivedData folder, so two peers never share build products. Never open another session's worktree in Xcode.
...
- Review and verification prompts are read-only on every existing checkout. If the gate must run on a branch, or a red/green check needs a source edit, use a throwaway worktree under the session scratchpad and remove it afterward.
...
- The owner's `iPhone 17` on iOS 27.0 holds the dev app's data and is where the restore drill in `.claude/rules/grdb.md` runs. Never seed a database into it, never erase it, never recreate it, and never install a peer's build on it. `scripts/gate` without `JUSTCHILL_SIMULATOR` targets it by name, which is why a peer always sets the variable.
- Each peer creates its own simulator, one per session, the same device on the same runtime: `xcrun simctl create justchill-<name> "iPhone 17" "iOS27.0"` prints the udid, `xcrun simctl boot <udid>` starts it. Every `simctl` call and every build names that udid. Never two sessions on one simulator: two `xcodebuild test` runs on one device fight over the test runner, and an install lands on whatever the other session is showing. *Why: on 2026-09-18 four peers queued on the owner's emulator in the predecessor repo, then a second read-only instance failed because the first boot lacked the flag; the owner: one device per agent.*
- A peer's simulator carries no sample data: create what the screen needs through the app.
- The Xcode MCP bridge (`claude mcp add --transport stdio xcode -- xcrun mcpbridge`) is for the owner's interactive session only: it drives the Xcode GUI, which must be open, and it sees one workspace. Peers use `xcrun simctl` and `scripts/*`.
- When the review cycle closes, the peer shuts its own simulator down and deletes it with `xcrun simctl delete <udid>`, together with the worktree cleanup. Never `delete all` or `erase all` (both denied), never another session's udid. Several simulators plus several `xcodebuild` runs is where the Mac saturates: the orchestrator staggers the boots instead of queueing the checks.

## Review cycle

- A session gets nothing new until its previous PR is reviewed, fixed and merged. Never queue two tickets in one dispatch.
- Two-axis review (standards vs. `CLAUDE.md` and `.claude/rules/`, spec vs. the issue), plus screenshots checked against `.claude/rules/ui-components.md`. Every PR gets one. Reviews of mechanical slices, such as restyles, docs and renames, run sonnet:medium. Reviews of screens, logic, migrations, backup/restore, auth and the composition root run opus:high. Post-review fixes run sonnet:low.
- Every PR review is a fresh `pr-reviewer` subagent that the orchestrator launches, one per PR, with the model passed on the call. It is not a long-lived peer session. The subagent is read-only. It does not build the app or rerun the gate, because CI already did. It reads the diff, the issue and `gh pr checks`, and it pulls the screenshots locally with `git show origin/assets/<N>-visual-check:<file>`. It checks that the SHA in each file name matches the PR head, and then views the images. It boots a simulator of its own only when screenshots are missing, stale or suspicious, and it never reports the peer's own simulator as a spec violation. It treats a base behind `origin/trunk` as a minor note, because the repo merges with rebase, and it rechecks an `IN_PROGRESS` CI job instead of failing the PR on it. *Why: on 2026-09-18 a review of PR #223 returned three blockers and all three were false, one of them the orchestrator's own device override.* It follows the `mattpocock-skills:code-review` skill for the Standards and Spec axes, with the PR's merge-base with `origin/trunk` as the fixed point. It returns `blocking|minor` findings, one verdict (MERGE or FIX FIRST) and one cause word (`checklist`, `judgment`, `spec`) for the dispatch log.
...
- Waiting on CI is two steps, both in the background, never a hand-written `until`. First prove the run exists: `gh api 'repos/emm3000/justchill-ios/actions/runs?head_sha=<sha>' --jq .total_count`, bounded by `for i in $(seq 1 N); do ...; sleep S; done`. A `0` that never moves means GitHub dropped the event for that push, and the fix is a new push, not more waiting. Then block on the run itself with `gh run watch <id> --exit-status` (or `gh pr checks <N> --watch --fail-fast`), which ends on its own; `gh pr checks` without a run reports "no checks reported" and exits at once, so it is no substitute for step one.
- Every background wait carries a hard bound, and on expiry the session reports instead of sleeping on. *Why: on 2026-09-17 a peer slept an hour on `/tmp/pr.txt`, a file nothing creates, and on a run that was never emitted.*
