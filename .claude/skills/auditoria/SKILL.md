...
ACTIVATE: existing target, no fresh diff — feature flow, package, `.github/`, docs vs code, project state.
...
- Never build or test — no `scripts/gate`, no `scripts/build`, no `xcodebuild`, no `swift build` or `swift test`. A finding only a build can prove is UNPROVEN, plus the exact command for the main thread.
