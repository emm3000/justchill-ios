...
- Never touch the owner's `iPhone 17` simulator. Each peer creates, uses and deletes its own `justchill-<name>` simulator.
- No wave carries two tickets touching the same package, and a schema change (a new migration) is always a wave of one.
...
| 2 | Code where the compiler or a test catches the error: one screen, a model rule, a use case | sonnet:medium | load `mattpocock-skills:tdd` for behavior; visual check on the peer's own simulator for a screen |
| 3 | Code on the trap list, where nothing catches the error: a `Route` case and its door, an `AppContainer` factory, `*Model.swift` purity beyond the SwiftUI import, an atom default that changes N screens, `project.yml`, a `Package.swift` edge, `.github/` | opus:medium | visual check of every affected screen |
| 4 | Migration, backup/restore, auth, the composition root, cross-package architecture | opus:high | the restore drill in `.claude/rules/grdb.md` when the schema moves |

Reviews and other roles keep the playbook rules: a `pr-reviewer` on every PR, restyle/docs/rename reviews sonnet:medium, screen/logic/migration/backup/auth/composition-root reviews opus:high, post-review fixes sonnet:low, `ticket-writer` opus:high, design fable.
...
- A package-wide sweep that needs judgment per line (which comment survives, which rationale is a duplicate) takes row 3, opus:medium. In the predecessor repo, wave #129-#133 ran it on sonnet:medium: all 5 PRs came back FIX FIRST, 3 for judgment (partial sweeps, kept history, repeated rationale).
- A dispatch that names a reference file to model the work on inherits that file's debt, so name its known gaps in the same breath. In the predecessor repo, wave #208-#220 pointed three tickets at one dialog whose `type` property carried no explicit type: #223 and #224 both came back FIX FIRST on exactly that line. Raising the row would have been the wrong lesson, because the model was not the cause.
...
5. Send each peer one dispatch built from the playbook checklist: issue, docs to read, branch `<type>/<n>-<slug>`, its worktree `../justchill-<name>`, its simulator `justchill-<name>` created with `xcrun simctl create`, the acceptance-criteria line, the gate (`JUSTCHILL_SIMULATOR=<udid> scripts/gate`), TDD or visual check per the table, `Closes #<n>`, no merge, reply with the PR URL. Ask for `notify_when_idle`.
